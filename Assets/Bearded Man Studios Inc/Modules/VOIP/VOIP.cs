    ...
    public class VOIP
    {
        ...
        private void Setup(string hostAddress, ushort port)
        {
            ...
            if (Socket.IsServer)
            {
                ...
                Socket.playerConnected += (player, sender) =>
                {
                    BMSLog.Log("PLAYER CONNECTED " + player.IPEndPointHandle.Address);
                };
                Socket.playerAccepted += (player, sender) =>
                {
                    BMSLog.Log("PLAYER ACCEPTED " + player.IPEndPointHandle.Address);
                };
                Socket.playerRejected += (player, sender) =>
                {
                    BMSLog.Log("PLAYER REJECTED " + player.IPEndPointHandle.Address);
                };
                Socket.playerDisconnected += (player, sender) =>
                {
                    BMSLog.Log("PLAYER DISCONNECTED " + player.IPEndPointHandle.Address);
                };
                StartVOIP(Socket);
            }
            else
        ...
        private void VOIPWorker()
        {
            while (Socket.IsConnected)
            {
                if (writeFlushTimer >= WRITE_FLUSH_TIME && writeSamples.Count > 0)
                {
                    ...
                    Binary voice = new Binary(Socket.Time.Timestep, false, writeBuffer, Receivers.All,
                        MessageGroupIds.VOIP, false);
