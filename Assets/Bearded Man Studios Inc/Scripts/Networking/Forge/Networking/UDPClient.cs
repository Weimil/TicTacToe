    ...
    public class UDPClient : BaseUDP, IClient
    {
        ...
        public override void Disconnect(bool forced)
        {
            ...
            lock (Client)
            {
                ...
                {
                    ConnectionClose frame = new ConnectionClose(Time.Timestep, false, Receivers.Server,
                        MessageGroupIds.DISCONNECT, false);
                    Send(frame, true);
        ...
        private void ReadNetwork()
        {
            ...
            try
            {
                ...
                while (IsBound)
                {
                    ...
                    {
                        if (packet.Size == 1 &&
                            (packet[0] == SERVER_BROADCAST_CODE || packet[1] == CLIENT_BROADCAST_CODE))
                        {
                        }
                        else if (packet.Size.Between(2, 4) && packet[0] == BROADCAST_LISTING_REQUEST_1 &&
