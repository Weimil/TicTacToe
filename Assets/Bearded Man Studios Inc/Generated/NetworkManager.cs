...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is GameNetworkNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					...
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						...
						{
							var go = Instantiate(GameNetworkNetworkObject[obj.CreateCode]);
			...
			else if (obj is NetBoardManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					...
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetBoardManagerNetworkObject.Length > 0 && NetBoardManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetBoardManagerNetworkObject[obj.CreateCode]);
		...
		public GameNetworkBehavior InstantiateGameNetworkNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameNetworkNetworkObject[index]);
			var netBehavior = go.GetComponent<GameNetworkBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
		...
		public NetBoardManagerBehavior InstantiateNetBoardManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetBoardManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetBoardManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
		...
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public GameNetworkBehavior InstantiateGameNetwork(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(GameNetworkNetworkObject[index]);
			var netBehavior = go.GetComponent<GameNetworkBehavior>();
		...
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NetBoardManagerBehavior InstantiateNetBoardManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetBoardManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<NetBoardManagerBehavior>();
